...
class MetricsTop():
    def __init__(self, train_mode):
        if train_mode == "regression":
            self.metrics_dict = {
                ...
                'META-SIMS': self.__eval_meta_sims_regression,
                'META-SIMSV2': self.__eval_meta_sims_regression,
                'META-MOSI': self.__eval_meta_mosi_regression,
                'META-MOSEI': self.__eval_meta_mosei_regression,
            ...
            self.metrics_dict = {
                ...
                'META-SIMS': self.__eval_meta_sims_classification,
                'META-SIMSV2': self.__eval_meta_sims_classification,
                'META-MOSI': self.__eval_meta_mosi_classification,
                'META-MOSEI': self.__eval_meta_mosei_classification,
            }
    ...
    def __eval_meta_sims_classification(self, y_pred, y_true):
        return self.__eval_mosi_classification(y_pred, y_true)
    def __eval_meta_mosi_classification(self, y_pred, y_true):
        return self.__eval_mosi_classification(y_pred, y_true)
    def __eval_meta_mosei_classification(self, y_pred, y_true):
        return self.__eval_mosi_classification(y_pred, y_true)
    ...
    def __eval_sims_regression(self, y_pred, y_true):
        ...
        return eval_results
    def __eval_meta_sims_regression(self, y_pred, y_true):
        test_preds = y_pred.view(-1).cpu().detach().numpy()
        test_truth = y_true.view(-1).cpu().detach().numpy()
        test_preds = np.clip(test_preds, a_min=-1., a_max=1.)
        test_truth = np.clip(test_truth, a_min=-1., a_max=1.)
    ...
    def __eval_meta_mosei_regression(self, y_pred, y_true, exclude_zero=False):
        return self.__eval_mosei_regression(y_pred, y_true)

    def __eval_meta_mosi_regression(self, y_pred, y_true):
        return self.__eval_mosei_regression(y_pred, y_true)
