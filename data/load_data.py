...
__all__ = ['MMDataLoader_meta']

logger = logging.getLogger('MSA')

def MMDataLoader_meta(args):
    datasets = {
        'query': MMDataset_meta(args, mode='query'),
        'train': MMDataset_meta(args, mode='train'),
        'valid': MMDataset_meta(args, mode='valid'),
        'test': MMDataset_meta(args, mode='test'),
...
class MMDataset_meta(Dataset):
    def __init__(self, args, mode='train'):
        # 每一个子集都有调用 generate_m
        self.mode = mode
        self.args = args
        DATA_MAP = {
            'meta-sims': self.__init_meta_sims,
            'meta-simsv2': self.__init_meta_simsv2,
            'meta-mosi': self.__init_meta_mosi,
            'meta-mosei': self.__init_meta_mosei,
        }
        DATA_MAP[args.datasetName]()

    def __init_meta_sims(self):
        ...
        if self.args.datasetName == 'meta-sims'or self.args.datasetName == 'meta-simsv2':
            for m in "TAV":
                self.labels[m] = data[self.mode][self.args.train_mode + '_labels_' + m].astype(np.float32)
        if (self.args.datasetName == 'meta-mosi' or self.args.datasetName == 'meta-mosei') and self.mode=='query':
            for m in "TAV":
                self.labels[m] = np.array(data[self.mode][self.args.train_mode + '_labels'])
    ...
    def __init_meta_simsv2(self):
        return self.__init_meta_sims()

    def __init_meta_mosi(self):
        return self.__init_meta_sims()

    def __init_meta_mosei(self):
        return self.__init_meta_sims()
